...
- 字段可独立设置是否覆盖更新。
- 每本书只对应一个 Notion 页面，书籍字段写入页面属性，划线与想法写入“微信读书划线与想法”一级标题下的托管区域。
- 重复同步会分页读取并比对托管区域，跳过未变化的内容，只更新差异；新增内容批量写入。旧版托管折叠块会在同步时迁移。
- 托管区域延续到下一个一级标题之前。请把手工内容放在托管标题之前，或另一个一级标题下，避免被同步覆盖。
...
7. 同步后，划线与想法会出现在对应书籍页面的“微信读书划线与想法”标题下。

增量同步仍需读取 Notion 中的现有块，以识别页面改动和上次未完成的写入，不依赖本地缓存判断是否同步完成。写入按最多 100 个块分批，同时控制 UTF-8 请求大小；遇到 429 时按 `Retry-After` 等待，最多重试 3 次。限制依据 [Notion 请求限制](https://developers.notion.com/reference/request-limits)。
